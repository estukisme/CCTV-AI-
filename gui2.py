...
cnn = tf.keras.models.load_model(
    "cnn/model_cnn_final_augmented.h5",
    compile=False
)
...
def buzzer_loop():
    ...
    while buzzer_running:
        ...
        time.sleep(2)   # jeda 2 detik sebelum ulang lagi
...
def detect_objects(frame):
    ...
    if roi.size != 0:
        ...
        raw_conf = float(np.max(pred)) * 100
        confidence = (0.7 * last_confidence) + (0.3 * raw_conf)
        confidence = min(confidence, 99.9)
        last_confidence = confidence

        is_red = (cid == 0)

        detected_label = "MERAH" if is_red else "TIDAK MERAH"
        status_code = 1 if is_red else 0

        display_text = f"{detected_label} ({confidence:.1f}%)"

        color_box = (0,0,255) if is_red else (0,255,255)
...
# CAMERA INIT
# =========================================================
rtsp_url = "rtsp://admin:Phonska1bisa@192.168.110.3:554/Streaming/Channels/101"
